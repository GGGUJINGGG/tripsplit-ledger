...
- **Password reset and trip invite emails aren't actually emailed** — no transactional email provider (SES, Resend, SendGrid, ...) is configured, so `POST /auth/forgot-password` and `POST /trips/{id}/participants/invite` both log the link server-side instead of sending it. The rest of each flow (single-use, hashed, time-limited reset tokens; pending trip invites that require explicit acceptance) is real and tested; only the delivery mechanism is a stand-in.
